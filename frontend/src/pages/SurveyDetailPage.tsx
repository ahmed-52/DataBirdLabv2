...
import { ArrowLeft, Calendar, FileText, Image as ImageIcon, MapPin, Search, Mic, Activity, Eye, Grid } from "lucide-react"
...
export default function SurveyDetailPage() {
  ...
  if (!survey) return <div className="p-12 text-center font-mono text-zinc-400">SURVEY_DATA_NOT_FOUND</div>
  ...
  return (
    <div className="min-h-screen bg-white pb-12">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                {(survey.status === 'processing' || survey.status === 'pending') ? (
                  <Badge variant="outline" className="rounded-sm border-orange-200 bg-orange-50 text-orange-700 text-[9px] uppercase font-bold tracking-widest px-1.5 py-0.5 pointer-events-none flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse"></span>
                    Processing Survey
                  </Badge>
                ) : survey.status === 'failed' ? (
                  <Badge variant="outline" className="rounded-sm border-red-200 bg-red-50 text-red-700 text-[9px] uppercase font-bold tracking-widest px-1.5 py-0.5 pointer-events-none">
                    Survey Failed
                  </Badge>
                ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="bg-zinc-100 rounded-sm border border-zinc-200 overflow-hidden relative">
              <div className="absolute top-3 left-3 z-[100] bg-white/90 backdrop-blur border border-zinc-200 px-2 py-1 rounded-sm flex items-center gap-2 shadow-none pointer-events-none">
                ...
                <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-700 font-display">Survey Map</span>
              </div>
                    ...
                    filteredDetections.map((det: any) => (
                      <TableRow key={`${det.type}-${det.id}`} className="group hover:bg-zinc-50 border-zinc-100">
                        ...
                        <TableCell className="py-1 text-right">
                          <div className="w-6 h-6 rounded-sm bg-zinc-100 border border-zinc-200 flex items-center justify-center ml-auto text-zinc-400">
                            {det.type === 'visual' ? <Eye size={10} /> : <Mic size={10} />}
                          </div>
                        </TableCell>
                      </TableRow>
                    ))
          ...
          <div className="space-y-6">
            <div className="bg-zinc-50 border border-zinc-200 rounded-sm p-4">
              <h4 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-3 border-b border-zinc-200 pb-2">Survey Summary</h4>
            ...
            {/* View Detections Button - Only for drone surveys */}
            {survey.type === 'drone' && visualDetections.length > 0 && (
              <Button
                onClick={() => setIsInspectorOpen(true)}
                className="w-full bg-zinc-900 hover:bg-zinc-800 text-white rounded-sm font-bold uppercase text-xs h-9 tracking-wide"
              >
      ...
      <InspectorPanel
        ...
        selectedVisual={null}
        selectedAcoustic={null}
        selectedARU={null}
        selectedSurvey={survey.type === 'drone' ? survey : null}
        filterDays={3650}
        selectedSurveyIds={[survey.id]}
      />
    </div>
  )
}
